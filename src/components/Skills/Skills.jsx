...
function Skills({ skillsRef }) {
  ...
  const [imgRef, fadeInImg] = useInView({ rootMargin: "100px 0px 0px 0px", threshold: 0, triggerOnce: true });
  const [barsRef, animateBars] = useInView({ rootMargin: "100px 0px 0px 0px", threshold: 0, triggerOnce: true });

  return (
    <section
      id="skill"
      className="md:relative md:h-dvh overflow-hidden bg-linear-to-b from-[#013c57] to-[#101f4c]"
    >
      <div
        ref={skillsRef}
      ...
      <img ref={imgRef} loading="lazy"
        className={`max-w-full w-full object-cover object-center transition-opacity duration-800 ease-in-out ${fadeInImg ? "opacity-50" : "opacity-0"}`}
      ...
      >
        <div className="text-center content-end w-full md:col-span-2">
          <span ref={barsRef} className="text-2xl md:text-3xl font-bold">Skills</span>
          <div className="w-full h-[1px] bg-[var(--white40)] mt-2 md:!my-4"></div>
        </div>
        <div className="w-full md:w-8/12 justify-center">
          <span className="tracking-widest block font-semibold mb-4 text-sm md:text-base">
            Coding
          </span>
          {animateBars && codingSkills.map((skill, index) => (
            <ProgressBarItem key={index} skill={skill} />
          ))}
        </div>
        <div className="w-full md:w-8/12 justify-center">
          <span className="tracking-widest block font-semibold text-sm md:text-base mb-4">
            Database & Design
          </span>
          {animateBars && dbSkills.map((skill, index) => (
            <ProgressBarItem key={index} skill={skill} />
          ))}
        </div>
      </div>
